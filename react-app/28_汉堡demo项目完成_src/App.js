  ...
  switch (action.type) {
    ...
    case 'SUB':
      ...
      if (action.meal.amount === 0) {
        let index = newCart.items.findIndex((e) => e.id === action.meal.id)
        if (index !== -1) {
    ...
    case 'CLEAR':
      ...
      // 将购物车的商品数量清零
      newCart.items.forEach((e) => delete e.amount)
      newCart.items = []
...
*/
  ...
  */
  ...
  const FilterHandler = (keyword) => {
    console.log(`output->mealsData`, mealsData)
    const newMealsData = MEALS_DATA.filter(
      (e) => e.title.indexOf(keyword) !== -1
    )
  ...
  return (
    <CarContext.Provider
      value={{ ...cartData, cartDataDispatch }}
    >
